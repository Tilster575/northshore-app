...
function generateJobRef(activity, scheduledDate, existingJobs) {
  ...
  const existing = existingJobs
    .filter((j) => j.job_ref && j.job_ref.startsWith(prefix))
    .map((j) => parseInt(j.job_ref.split("-").pop(), 10))
    .filter((n) => !isNaN(n));
  const next = existing.length > 0 ? Math.max(...existing) + 1 : 1;
  return `${prefix}${String(next).padStart(3, "0")}`;
}

/* Map DB row (snake_case) → display object */
function mapJob(row) {
  return {
    id: row.id,
    jobRef: row.job_ref,
        ...
        <div style={{ padding: 24, maxWidth: 700, margin: "0 auto" }}>
          ...
          <div style={{ background: CARD, border: `2px solid ${ACCENT}`, borderRadius: 16, padding: 28 }}>
            ...
            <div style={{ background: "#1a1a1a", border: `1px solid ${BORDER}`, borderRadius: 12, padding: 16, marginBottom: 24 }}>
              ...
              <input ref={compFileRef} type="file" accept="image/*" capture="environment" style={{ display: "none" }} onChange={handleCompPhoto} />
        ...
        <div style={{ padding: 24, maxWidth: 700, margin: "0 auto" }}>
          ...
          <div style={{ background: CARD, border: `1px solid ${isCancelled ? "#ef444444" : BORDER}`, borderRadius: 16, padding: 28 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 20 }}>
              {[["Customer", job.customerName], ["Telephone", job.phone], ["Location", job.location], ["Activity", job.activity], ["Scheduled Date", formatScheduledDate(job.scheduledDate)], ["Priority", job.priority]].map(([l, v]) => (
                <div key={l}>
